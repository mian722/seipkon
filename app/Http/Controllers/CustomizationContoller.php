...
class CustomizationContoller extends Controller
{
    public function index()
    {
        ...
        $domaindetails = DomainlistModel::where('admin_id',Auth::user()->id)->get();
        return view('admin.customization',compact('smtpdetail','domaindetails'));
    }
...
public function networklogo(Request $request)
    ...
    {
    	$checkExist = DomainlistModel::where('admin_id', Auth::user()->id)->where('id', $request->domainid)->first();
    	if ($checkExist) {
    		$update = DB::table('trackingdomain')
            ->where('admin_id', Auth::user()->id)
            ->where('id', $request->domainid)
            ->update(['admin_id' => Auth::user()->id,'domain' => $request->domainname,'sslstatus' => $request->sslstatus,'status' => $request->domainstatus,'is_default' => $request->is_default]);
            if(!empty($update)){
	            return 'Succfully Updated!';
	        }else{
    	...
    	{
    	$assignoffer = new DomainlistModel;
	        $assignoffer->admin_id = Auth::user()->id;
	        $assignoffer->domain = $request->domainname;
	        $assignoffer->sslstatus = $request->domainstatus;
	        $assignoffer->status = $request->sslstatus;
	        $assignoffer->save();
	        if(!empty($assignoffer)){
	            return redirect()->back()->with('success','Succfully Added!');
	        }else{
    ...
    {
    	return $domaindetail = DomainlistModel::find($id);
